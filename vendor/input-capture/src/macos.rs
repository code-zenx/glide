...
struct InputCaptureState {
    ...
    bounds: Bounds,
    /// glide: which display the barrier belongs to, as an index into the
    /// active display list. `None` means the union of every display, which is
    /// what upstream always did. With a value, an edge belongs to that one
    /// screen, so a peer can sit above a display that is not the topmost.
    only_display: Option<usize>,
...
impl InputCaptureState {
    ...
    fn crossed(&mut self, event: &CGEvent) -> Option<Position> {
        ...
        for &position in self.active_clients.iter() {
            // glide: the cursor must be on the barrier's own screen, otherwise
            // a taller neighbour would trip a barrier that is not its own.
            let along_x = location.x >= self.bounds.xmin && location.x <= self.bounds.xmax;
            let along_y = location.y >= self.bounds.ymin && location.y <= self.bounds.ymax;
            if (position == Position::Left && (location.x + relative_x) <= self.bounds.xmin && along_y)
                || (position == Position::Right && (location.x + relative_x) >= self.bounds.xmax && along_y)
                || (position == Position::Top && (location.y + relative_y) <= self.bounds.ymin && along_x)
                || (position == Position::Bottom && (location.y + relative_y) >= self.bounds.ymax && along_x)
            {
    ...
    // Get the max bounds of all displays, or of one chosen display.
    fn update_bounds(&mut self) -> Result<(), MacosCaptureCreationError> {
        ...
        // glide: narrow the list to the chosen screen when there is one.
        let chosen: Vec<u32> = match self.only_display.and_then(|i| active_ids.get(i).copied()) {
            Some(id) => vec![id],
            None => active_ids.clone(),
        };
        chosen.iter().for_each(|d| {
        ...
        log::debug!(
            "Updated displays bounds: {0:?} (display {1:?} of {2})",
            self.bounds,
            self.only_display,
            active_ids.len()
        );
    ...
    /// glide: put later barriers on this display only.
    fn set_only_display(&mut self, index: Option<usize>) {
        self.only_display = index;
        if let Err(e) = self.update_bounds() {
            log::warn!("cannot read display bounds: {e}");
        }
    }
...
pub struct MacOSInputCapture {
    ...
    notify_tx: Sender<ProducerEvent>,
    run_loop: CFRunLoop,
    /// glide: shared with the event tap thread, so the barrier can be moved to
    /// a single display after the capture is already running.
...
impl Capture for MacOSInputCapture {
    /// glide: confine the barriers to one display.
    async fn restrict_to_display(&mut self, index: Option<usize>) {
        self.state.lock().await.set_only_display(index);
    }
