...
impl InputCapture {
    ...
    /// glide: put the barriers on a single display, by index into the
    /// platform's display list. `None` restores the union of every display.
    pub async fn restrict_to_display(&mut self, index: Option<usize>) {
        self.capture.restrict_to_display(index).await;
    }
...
trait Capture: Stream<Item = Result<(Position, CaptureEvent), CaptureError>> + Unpin {
    /// glide: put the barriers on one display instead of the union of all of
    /// them, where the backend can express that. Backends that cannot simply
    /// ignore it.
